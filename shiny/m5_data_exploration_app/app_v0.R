...
plot_data <- function(ts_data, calendar, min_date, max_date, input){
    
    ## Test
    
    # ts_data = sales_aggregation_level_8[1,]
    # min_date = as.Date("2014-04-14")
    # max_date = as.Date("2015-04-14")
    
    min_date <- as.Date(min_date)
    max_date <- as.Date(max_date)
    
    # Keep the required dates from calendar
    short_calendar <- calendar %>% filter(date >= min_date, date <= max_date)
    ...
    # Create data frame
    df <- data.frame(dates = short_calendar$date,
                     snap_CA = short_calendar$snap_CA,
                     snap_TX = short_calendar$snap_TX,
                     snap_WI = short_calendar$snap_WI,
                     event_type = short_calendar$event_type_1,
                     sales = ts_data %>% select(!!(short_calendar$d)) %>% as.numeric())
...
server <- function(input, output) {
    ...
    output$sales_plot <- renderPlotly({
        ...
        if(input$hierarhy_lvl == "Total"){# hierarhy_lvl 1
            ...
            p <- plot_data(ts_data = sales_aggregation_level_1, calendar = calendar, min_date = input$daterange[1], max_date = input$daterange[2], input)
            ...
            p <- plot_data(ts_data = ts_data, calendar = calendar, min_date = input$daterange[1], max_date = input$daterange[2], input)
