...
# The R code can be used for implementing the M5 benchmarks for the case of the uncertainty forecasting
# The code describes - among others - the way the weights are calculated per series, the scaling applied,
# as well as how the final scores are computed per level and in total.
# Given that the validation test-set is not provided to the participants at this stage of the competition,
# a dummy test-set is given instead to enable the calculation of the scores.
...
statistics <- function(tsid){
  ...
  cat("\r", input$Hlevel, input$Agg_Level_1, input$Agg_Level_2)
...
benchmarks_f <- function(x, fh){
  ...
  cat("\r", x$Hlevel, x$Agg_Level_1, x$Agg_Level_2)
...
time_series_b <- readRDS("data/processed/time_series_b_uncertainty.rds")

# Level 1
ex_sales <- sales
sales_train <- ex_sales[,7:ncol(ex_sales)] 
...
## 05. Estimate statistics and dollar sales used for weighting  ----

time_series_b <- readRDS("data/processed/time_series_b_uncertainty.rds")

cl = registerDoSNOW(makeCluster(5, type = "SOCK", outfile = ""))

stat_total <- foreach(tsi=1:length(time_series_b), .combine='rbind') %dopar% statistics(tsi)

saveRDS(object = stat_total, file = "data/processed/stat_total_complete.rds")


## 06. Estimate forecasts for benchmark methods ----

time_series_b <- readRDS("data/processed/time_series_b_uncertainty.rds")

qlist <- c(0.005, 0.025, 0.165, 0.25, 0.5, 0.75, 0.835, 0.975, 0.995)

b_names <- c("Naive", "sNaive", "SES", "ETS", "ARIMA", "Kernel") # 6 benchmark methods

cl = registerDoSNOW(makeCluster(6, type = "SOCK", outfile = ""))
...
View(frc_total)
saveRDS(object = frc_total, file = "data/forecastings/uncertainty/frc_benchmarks_top_100_ts.rds")


## 07. Calculate time_series_b_out object -----

sales_out <- read.csv("data/raw/sales_test_validation.csv", stringsAsFactors = F)
weights <- read.csv("data/raw/weights_validation.csv", stringsAsFactors = F) #based on last 28 days to compute WRMSSE
...
saveRDS(object = time_series_b_out, file = "data/processed/time_series_b_out.rds")


## 08. Calculate final scores ----

time_series_b <- readRDS("data/processed/time_series_b_uncertainty.rds")

time_series_b_out <- readRDS("data/processed/time_series_b_out.rds")

frc_total <- readRDS("data/forecastings/uncertainty/frc_benchmarks_top_100_ts.rds")
...
for (bid in b_names){
  ...
  for (i in 1:length(time_series_b)){
    ...
    for_temp <- frc_total[(frc_total$Agg_Level_1==insample$Agg_Level_1) & (frc_total$Agg_Level_2==insample$Agg_Level_2),
                          c("fh","quantile",bid)]
...
## 09. Export benchmarks' forecasts in Kaggle format ----

frc_total <- readRDS("data/forecastings/uncertainty/frc_benchmarks_top_100_ts.rds")

template <- read.csv("data/submissions/sample_submission.csv", stringsAsFactors = F)
