...
calendar = pd.read_csv(os.path.join("calendar.csv"))
selling_prices = pd.read_csv(os.path.join("sell_prices.csv"))
sample_submission = pd.read_csv(os.path.join("sample_submission.csv"))

sales = pd.read_csv(os.path.join("sales_train_validation.csv"))
...
def create_model(lr=0.002):
    ...
    x = Dense(73, activation="tanh")(x)
    x = Dense(71, activation="tanh")(x)
    x = Dense(29, activation="tanh")(x)
    outputs = Dense(1, activation="softplus", name='output')(x)
...
model = load_model('Softpluswillsaveus.h5')
...
model.save('Softpluswillsaveus.h5')

pred = model.predict(X_test, batch_size=10001) * 1.112
test["demand"] = pred.clip(0)
submission = test.pivot(index="id", columns="F", values="demand").reset_index()[sample_submission.columns]
submission = sample_submission[["id"]].merge(submission, how="left", on="id")
submission.head()
submission[sample_submission.id=="FOODS_1_001_TX_2_validation"].head()
submission.to_csv("submission_NN_01.csv", index=False)
